...
const core = {}
const host_leshan = "http://localhost:8080"
...
core.get_all_node = async () => {
    const response = await fetch_get(`${host_leshan}${list_apis.get_all_node_name}`)
    const list_node_name = response.body;
...
core.get_all_info = async () => {
    ...
    return await Promise.all(all_node.map(async node => {
        ...
        const list_url = _.map(objectLinks, object => object.url)
        const list_object_links = _.reject(objectLinks, obj => !_.includes(list_url, obj.url));
