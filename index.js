...
async function remote_pump_auto(payload_config) {
  ...
  _.forEach(static_list_garden, async static_garden => {
    ...
    if (!!list_humidity && list_humidity.length === 0) return consoleCatch({ code: `get_list_humidity_in_garden${garden_index}_empty` })
    const _list_humidity_vals = list_humidity.map(i => i.value)
    if (!!_list_humidity_vals && _list_humidity_vals.length === 0) return consoleCatch({ code: `get_list_humidity_in_garden${garden_index}_empty` })
